...
call_gcc()
{
  ...
  export ANDROID_SYSROOT="${ANDROID_NDK_HOME}/platforms/${_ANDROID_PLATFORM}/${_ANDROID_ARCH}"
  ANDROID_TOOLCHAIN=""
  for host in "linux-x86_64" "linux-x86" "darwin-x86_64" "darwin-x86"; do
    if [[ -d "${ANDROID_NDK_HOME}/toolchains/${_ANDROID_EABI}-4.9/prebuilt/${host}/bin" ]]; then
      ANDROID_TOOLCHAIN="${ANDROID_NDK_HOME}/toolchains/${_ANDROID_EABI}-4.9/prebuilt/${host}/bin"
      break
    fi
  done
